...
public:
	...
	void Print(vector<int>& ans, node* a)
	{
		if (a == NULL)
			return;
		a->push();
...
int32_t main()
{
	...
	for (int i = 0; i < Q; i++)
	{
		...
		if (a == 1)
		{
			...
			b++;
			c++;
			std::cout << splay.GetMin(b, c) << " ";
			std::cout << splay.GetMax(b, c) << " ";
			std::cout << splay.GetSum(b, c) << endl;
			splay.Reverse(b, c);
		}
		else if (a == 2)
		{
		...
		else if (a == 4)
		{
			...
			splay.Splay(n);
			if (n != NULL && n->l != NULL)
				std::cout << n->l->size << endl;
			else if (n->l == NULL)
				std::cout << 1 << endl;
		}
	ans.clear();
	splay.Print(ans, splay.root);
